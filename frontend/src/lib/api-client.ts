...
export const getApiBaseUrl = (): string => {
  // 統一ストレージから取得、またはVite環境変数、デフォルトは開発環境URL
  const stored = getSubKey('apiBaseUrl')
...
export const fetch5chPosts = async (
  ...
  return apiFetch<FivechPost[]>(`/api/5ch/posts?thread_url=${encodedUrl}&start=${start}&end=${end}`)
}

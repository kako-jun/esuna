...
export default function RSSFeedList(props: RSSFeedListProps) {
  ...
  onMount(() => {
    setTimeout(() => {
      props.speech.speak(
        `${FORMAL_SERVICE_NAMES.rss} の一覧です。${defaultFeeds.length}個のニュースサイトを並べています。外部サイトから取得するため、失敗する場合があります。`,
      )
  ...
  const actions = () => {
    const actionList: GridAction[] = [
      ...
      },
      {
        ...
        action: () => {
          if (currentIndex() > 0) {
            ...
            setTimeout(() => {
              props.speech.speak(`${defaultFeeds[currentIndex()].name}`, { interrupt: true })
            }, 100)
          } else {
            props.speech.speak('最初のフィードです')
          }
        },
      },
      {
        ...
        action: () => {
          if (currentIndex() < defaultFeeds.length - 1) {
            ...
            props.speech.speak('最後のフィードです')
          }
        },
      },
      { label: '読み上げ', action: speakFeed },
      {
        label: defaultFeeds[currentIndex()] ? defaultFeeds[currentIndex()].name : 'フィードなし',
        action: () => {
          const feed = defaultFeeds[currentIndex()]
          if (typeof window !== 'undefined') {
            sessionStorage.setItem('selectedRSSFeed', JSON.stringify(feed))
          }
          props.speech.speak(
            `${feed.name} の記事一覧を開いています。外部サイトから取得するため、少し待ってください。`,
          )
          props.onSelectFeed()
        },
      },
      {
        ...
        action: () => {
          const feed = defaultFeeds[currentIndex()]
          props.speech.speak(`フィード番号 ${currentIndex() + 1}。名前：${feed.name}。`)
        },
      },
      {
        label: 'フィード数',
        action: () => {
          props.speech.speak(
            `全${defaultFeeds.length}フィード中、${currentIndex() + 1}番目のフィードです`,
          )
        },
      },
      {

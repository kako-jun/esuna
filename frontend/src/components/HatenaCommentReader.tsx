...
export default function HatenaCommentReader(props: HatenaCommentReaderProps) {
  ...
  const loadComments = async () => {
    ...
    try {
      ...
      props.speech.speak(
        'はてなブックマークのコメントを取得できませんでした。外部サービスへの接続に失敗しました。1番で戻るか、4番でもう一度試せます。',
      )
    } finally {
      setLoading(false)
    }
  ...
  const reload = () => {
    ...
    if (entry && entry.comments_url) {
      ...
      fetchHatenaComments(entry.comments_url)
        .then((comments) => {
          store.setHatenaComments(comments, entry.comments_url)
          props.speech.speak(`${comments.length}件のコメントを再読み込みしました`)
        })
        .catch(() => {
          props.speech.speak(
            'はてなブックマークのコメントを再取得できませんでした。外部サービスへの接続に失敗しました。1番で戻ってください。',
          )
        })
  ...
  const actions = () => {
    ...
    const actionList: GridAction[] = [
      ...
      },
      {
        label: '前のコメント',
        action: () => {
          if (store.state.currentCommentIndex > 0) {
            store.prevComment()
            setTimeout(speakComment, 100)
          } else {
            props.speech.speak('最初のコメントです')
          }
        },
      },
      {
      ...
      { label: '読み上げ', action: speakComment },
      {
        label: '位置',
        action: () =>
          props.speech.speak(
            `${store.state.hatenaComments.length}件中、${store.state.currentCommentIndex + 1}件目です`,
          ),
      },
      ...
      <GridSystem
        ...
        onInit={() => {
          ...
          if (store.state.hatenaComments.length > 0)
            props.speech.speak(`${store.state.hatenaComments.length}件のコメントがあります`)
        }}
      />
    </div>
  )

...
export default function RadioStationList(props: RadioStationListProps) {
  ...
  onMount(() => {
    setTimeout(() => {
      props.speech.speak(
        `${FORMAL_SERVICE_NAMES.radio} の一覧です。${allStations.length}局あります。NHKラジオ第2は時間帯により放送休止することがあります。radiko は未対応です。`,
      )
  ...
  const speakStation = () => {
    ...
    if (!station) return
    props.speech.speak(
      `${station.name}。${station.description}。ラジオ局番号 ${currentIndex() + 1}`,
      { interrupt: true },
    )
  }
  ...
  const actions = () => {
    ...
    const actionList: GridAction[] = [
      ...
      },
      {
        ...
        action: () => {
          if (currentIndex() > 0) {
            ...
            setTimeout(() => {
              props.speech.speak(`${allStations[currentIndex()].name}`, { interrupt: true })
            }, 100)
          } else {
            props.speech.speak('最初のラジオ局です')
          }
        },
      },
      {
      ...
      },
      {
        label: '局情報',
        action: () => {
          const s = allStations[currentIndex()]
          props.speech.speak(
            `ラジオ局番号 ${currentIndex() + 1}。名前：${s.name}。サービス：${getServiceName(s.service)}。説明：${s.description}`,
          )
        },
      },
      {
        label: allStations[currentIndex()] ? `${allStations[currentIndex()].name}` : '',
        action: speakStation,
      },
      {
        ...
        action: () => {
          ...
          props.speech.speak(
            `${station.name} を開いています。ブラウザによっては再生できない場合があります`,
          )
      ...
      },
      {
        label: '局数',
        action: () => {
          props.speech.speak(`全${allStations.length}局中、${currentIndex() + 1}番目のラジオ局です`)
        },
      },
      {
        label: '停止',

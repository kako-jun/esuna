...
#### 症状
- `停止` を1回押しただけで、その後の読み上げが戻らない
- `停止` を `一時停止` のつもりで押したが、続きから再生されない

#### 現在の仕様

- 現在の Esuna では、`停止` は `一時停止` ではない
- `停止` は現在の発話と後続キューを破棄する
- 続きからの `再開` ボタンはまだない

#### 対処法

- 同じ項目をもう一度選び直して最初から読み上げる
- 続き位置を保持している機能では、`続きから` メニューを使う

#### 補足

自動ナビゲーションが有効な画面では、`停止` と `読み終わり` の区別がまだ十分に整理されていない。今後の改善対象。
